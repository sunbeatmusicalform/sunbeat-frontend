...
export async function generateMetadata({
  ...
  return generateWorkflowMetadata({
    ...
    defaultDescription:
      "Preencha o formulário para cadastrar os dados da sua empresa.",
  });
}
...
export default async function CompanyLayout({
  ...
  const { workspaceSlug } = await params;
  const enabled = await isWorkflowEnabledForWorkspace(workspaceSlug, "company_registry");

  if (!enabled) {
    return <WorkflowDisabledScreen workflowLabel="Cadastro de empresa" />;
  }
