...
export default async function ClearanceLayout({
  children,
  params,
}: {
  ...
  const { workspaceSlug } = await params;
  const enabled = await isWorkflowEnabledForWorkspace(workspaceSlug, "rights_clearance");

  if (!enabled) {
    return <WorkflowDisabledScreen workflowLabel="Clearance de direitos" />;
  }
